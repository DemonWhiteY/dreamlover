...
def char_response(origin_text,initial_messages,meta_info):
    initial_messages.append({"role": "user", "content": origin_text})
    ...
    print(bot_response)
    initial_messages.append({"role": "assistant", "content": bot_response})
...
def message_receiver():
    ...
    conn = sqlite3.connect('../Client/RIKO_Data/StreamingAssets/Database.db')
    cursor = conn.cursor()

    ###
    #  添加另一个数据库连接
    conn1 = sqlite3.connect('ai-database.db')
    ...
    while not exit_event.is_set():
      try: 
        ...
        elif json_data.get("command") == "login":
                # 处理登录命令
                ask = json_data.get("ask")
                userid, password = ask.split('_')
                print("userid:"+userid+"password:"+password)
                # 在ai-database中查找用户名和密码
                cursor1.execute("SELECT id, password FROM users WHERE id = ?", (userid,))
                result = cursor1.fetchone()
                if result is None:
                    response = {"command": "echo", "response": "fail", "emotion": "1"}
                else:
                    stored_password = result[1]
                    if password != stored_password:
                        
                        response = {"command": "echo", "response": "fail", "emotion": "2"}
                    else:
                        response = {"command": "echo", "response": "success","emotion":"3"}
                print(response)
                # 发送响应回客户端
